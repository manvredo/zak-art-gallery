...
export default function ProductCard({ product, onClick, showAddToCart = false, index = 0 }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden rounded-xl mb-6 group-hover:shadow-[0_4px_16px_rgba(0,0,0,0.4)] group-hover:translate-y-[7px] group-hover:scale-[1.01] group-hover:outline group-hover:outline-3 group-hover:outline-[#565656] transition-transform duration-200 ease-in transition-shadow duration-200 ease-in will-change-transform">
        <img
          src={product.image}
          alt={product.name}

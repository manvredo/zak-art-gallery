...
export default function ProductCard({ product, onClick, showAddToCart = false, index = 0 }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden rounded-xl mb-3 group-hover:shadow-[0_4px_16px_rgba(0,0,0,0.4)] group-hover:scale-[1.03] group-hover:outline group-hover:outline-3 group-hover:outline-gray-300 transition-transform duration-200 ease-in transition-shadow duration-200 ease-in will-change-transform">
        <img
          src={product.image}
          alt={product.name}
          className="w-full h-full object-cover transition-transform duration-200 ease-in group-hover:scale-[1.02]"
        />
